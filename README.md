# loop-engineering-top-stars

**中文** | [English](./README.en.md) · [备选榜](./README.overflow.md)

> GitHub 高星 Loop Engineering 项目精选 — Spec 驱动 · Agent 迭代循环 · SDD 工作流  
> 来源：[wind8ai 的 loop-engineering 列表](https://github.com/stars/wind8ai/lists/loop-engineering) · 更新：2026-08-27

---

## Contents

01. [obra/superpowers (278k ⭐)](#1-obrasuperpowers-278k-)
02. [affaan-m/ECC (244k ⭐)](#2-affaan-mecc-244k-)
03. [mattpocock/skills (238k ⭐)](#3-mattpocockskills-238k-)
04. [multica-ai/andrej-karpathy-skills (208k ⭐)](#4-multica-aiandrej-karpathy-skills-208k-)
05. [github/spec-kit (132k ⭐)](#5-githubspec-kit-132k-)
06. [karpathy/autoresearch (95k ⭐)](#6-karpathyautoresearch-95k-)
07. [addyosmani/agent-skills (90k ⭐)](#7-addyosmaniagent-skills-90k-)
08. [Leonxlnx/taste-skill (81k ⭐)](#8-leonxlnxtaste-skill-81k-)
09. [Fission-AI/OpenSpec (66k ⭐)](#9-fission-aiopenspec-66k-)
10. [bmad-code-org/BMAD-METHOD (52k ⭐)](#10-bmad-code-orgbmad-method-52k-)
11. [OthmanAdi/planning-with-files (26k ⭐)](#11-othmanadiplanning-with-files-26k-)
12. [EveryInc/compound-engineering-plugin (25k ⭐)](#12-everyinccompound-engineering-plugin-25k-)
13. [snarktank/ralph (22k ⭐)](#13-snarktankralph-22k-)
14. [pydantic/pydantic-ai (20k ⭐)](#14-pydanticpydantic-ai-20k-)
15. [PrimeIntellect-ai/prime-agent (19k ⭐)](#15-primeintellect-aiprime-agent-19k-)
16. [hardikpandya/stop-slop (16k ⭐)](#16-hardikpandyastop-slop-16k-)
17. [mindfold-ai/Trellis (14k ⭐)](#17-mindfold-aitrellis-14k-)
18. [cobusgreyling/loop-engineering (11k ⭐)](#18-cobusgreylingloop-engineering-11k-)
19. [frankbria/ralph-claude-code (9.6k ⭐)](#19-frankbriaralph-claude-code-96k-)
20. [open-gsd/gsd-core (8.8k ⭐)](#20-open-gsdgsd-core-88k-)
21. [gsd-build/gsd-2 (7.8k ⭐)](#21-gsd-buildgsd-2-78k-)
22. [huangruiteng/loopx (5.2k ⭐)](#22-huangruitengloopx-52k-)
23. [gotalab/cc-sdd (3.6k ⭐)](#23-gotalabcc-sdd-36k-)
24. [mikeyobrien/ralph-orchestrator (3.1k ⭐)](#24-mikeyobrienralph-orchestrator-31k-)
25. [Forward-Future/loopy (3.1k ⭐)](#25-forward-futureloopy-31k-)
26. [michaelshimeles/ralphy (3k ⭐)](#26-michaelshimelesralphy-3k-)
27. [rpamis/comet (2.9k ⭐)](#27-rpamiscomet-29k-)
28. [humanlayer/advanced-context-engineering-for-coding-agents (2.6k ⭐)](#28-humanlayeradvanced-context-engineering-for-coding-agents-26k-)
29. [subsy/ralph-tui (2.4k ⭐)](#29-subsyralph-tui-24k-)
30. [maxritter/pilot-shell (2.1k ⭐)](#30-maxritterpilot-shell-21k-)
31. [777genius/agent-teams-ai (2k ⭐)](#31-777geniusagent-teams-ai-2k-)
32. [Th0rgal/open-ralph-wiggum (1.9k ⭐)](#32-th0rgalopen-ralph-wiggum-19k-)
33. [the-open-engine/zeroshot (1.8k ⭐)](#33-the-open-enginezeroshot-18k-)
34. [ghuntley/how-to-ralph-wiggum (1.7k ⭐)](#34-ghuntleyhow-to-ralph-wiggum-17k-)
35. [Priivacy-ai/spec-kitty (1.6k ⭐)](#35-priivacy-aispec-kitty-16k-)
36. [uber/ADR (1.5k ⭐)](#36-uberadr-15k-)
37. [umputun/ralphex (1.5k ⭐)](#37-umputunralphex-15k-)
38. [AnandChowdhary/continuous-claude (1.4k ⭐)](#38-anandchowdharycontinuous-claude-14k-)
39. [modu-ai/moai-adk (1.2k ⭐)](#39-modu-aimoai-adk-12k-)
40. [open-gsd/gsd-pi (1.2k ⭐)](#40-open-gsdgsd-pi-12k-)
41. [alchaincyf/loop-engineering-orange-book (1.1k ⭐)](#41-alchaincyfloop-engineering-orange-book-11k-)
42. [ClaytonFarr/ralph-playbook (1k ⭐)](#42-claytonfarrralph-playbook-1k-)

→ [备选榜（8 条）](./README.overflow.md)

---

## 1. obra/superpowers (278k ⭐)

**🔗** https://github.com/obra/superpowers  
**🍴** Fork 25k | **🔄** Updated 2026-08-19  
**👤** Jesse Vincent — Keyboardio 创始人，开源社区资深贡献者

> 完整的 coding agent 软件开发方法论与技能框架。先推导 spec，再以 subagent 驱动 TDD 开发循环，让 agent 自主高效工作数小时。

**匹配能力**：📋 Spec 驱动 · 🔀 多 Agent 编排 · 🧪 TDD 工作流 · 🤖 Agent 集成

---

## 2. affaan-m/ECC (244k ⭐)

**🔗** https://github.com/affaan-m/ECC  
**🍴** Fork 37k | **🔄** Updated 2026-08-25  
**👤** Affaan Mustafa — Anthropic 黑客松冠军，PMX 创始工程师

> Agent Harness 性能优化全栈系统，涵盖 Skills、本能、记忆、安全与研究优先开发流程，面向 Claude Code / Codex / Cursor 等。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 3. mattpocock/skills (238k ⭐)

**🔗** https://github.com/mattpocock/skills  
**🍴** Fork 20k | **🔄** Updated 2026-08-24  
**👤** Matt Pocock — TypeScript 教育 KOL，Total TypeScript 创始人

> Matt Pocock 的个人 skills 目录，直接从 .claude 目录整理，面向真正的工程师。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 4. multica-ai/andrej-karpathy-skills (208k ⭐)

**🔗** https://github.com/multica-ai/andrej-karpathy-skills  
**🍴** Fork 21k | **🔄** Updated 2026-04-20  
**👤** multica-ai — Multica coding agent 平台维护方，Karpathy 式 Claude Code 指南作者

> 基于 Andrej Karpathy 对 LLM 编程陷阱的观察整理的单文件 CLAUDE.md 指南，以「先思考再编码、简单优先、精准修改、目标驱动」四条原则改善 Claude Code 行为。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 5. github/spec-kit (132k ⭐)

**🔗** https://github.com/github/spec-kit  
**🍴** Fork 12k | **🔄** Updated 2026-08-27  
**👤** GitHub 官方团队 — Spec-Driven Development 工具链维护方

> Spec-Driven Development 入门工具包，帮助开发者快速上手 spec 驱动开发方法论。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 6. karpathy/autoresearch (95k ⭐)

**🔗** https://github.com/karpathy/autoresearch  
**🍴** Fork 13k | **🔄** Updated 2026-03-26  
**👤** Andrej Karpathy — 前 Tesla AI / OpenAI，nanochat 与 autoresearch 作者

> 单 GPU nanochat 训练的自主 AI 研究循环：agent 修改 train.py、跑 5 分钟实验、用 val_bpb 判断保留或丢弃，人类只迭代 program.md 编排研究组织。

**匹配能力**：🤖 Agent 集成

---

## 7. addyosmani/agent-skills (90k ⭐)

**🔗** https://github.com/addyosmani/agent-skills  
**🍴** Fork 9.6k | **🔄** Updated 2026-08-27  
**👤** Addy Osmani — Google Chrome 工程总监，《Learning JavaScript Design Patterns》作者

> 生产级 AI coding agent 工程技能集合，覆盖现代工程实践关键场景。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 8. Leonxlnx/taste-skill (81k ⭐)

**🔗** https://github.com/Leonxlnx/taste-skill  
**🍴** Fork 5.6k | **🔄** Updated 2026-08-24  
**👤** Leon Lin — Taste Skill 创始人

> 开源设计品味 Skill，为 AI coding agent 提供更好的布局、排版、间距与动效审美。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 9. Fission-AI/OpenSpec (66k ⭐)

**🔗** https://github.com/Fission-AI/OpenSpec  
**🍴** Fork 4.6k | **🔄** Updated 2026-08-26  
**👤** Fission-AI — SDD 框架开源团队

> 面向 AI coding assistant 的 Spec-Driven Development（SDD）框架。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 10. bmad-code-org/BMAD-METHOD (52k ⭐)

**🔗** https://github.com/bmad-code-org/BMAD-METHOD  
**🍴** Fork 6k | **🔄** Updated 2026-08-27  
**👤** BMAD 团队 — AI 驱动敏捷开发方法论开源组织

> Breakthrough Method for Agile AI Driven Development，AI 驱动敏捷开发的突破性方法论。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 11. OthmanAdi/planning-with-files (26k ⭐)

**🔗** https://github.com/OthmanAdi/planning-with-files  
**🍴** Fork 2.2k | **🔄** Updated 2026-08-22  
**👤** Ahmad Othman Adi (OthmanAdi) — migRaven 教育者，planning-with-files 持久化文件规划 Skill 作者

> 面向 AI coding agent 的持久化文件规划 Skill。用 Markdown 计划文件对抗上下文丢失与 /clear，附带确定性完成门禁，适合长周期 agentic 任务。

**匹配能力**：🤖 Agent 集成

---

## 12. EveryInc/compound-engineering-plugin (25k ⭐)

**🔗** https://github.com/EveryInc/compound-engineering-plugin  
**🍴** Fork 2k | **🔄** Updated 2026-08-27  
**👤** Every — Compound Engineering 方法与插件团队

> Every 的 Compound Engineering 官方插件，以可复用 Skills 将研究、规划、实现、审查和经验沉淀组织为持续复利的工程工作流，支持 Claude Code、Codex 与 Cursor。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 13. snarktank/ralph (22k ⭐)

**🔗** https://github.com/snarktank/ralph  
**🍴** Fork 2.1k | **🔄** Updated 2026-02-02  
**👤** snarktank — Ralph 自主 agent 循环框架作者

> 自主 Agent 循环框架：按 PRD 条目反复执行直至全部完成，适合长周期无人值守 feature 交付。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 14. pydantic/pydantic-ai (20k ⭐)

**🔗** https://github.com/pydantic/pydantic-ai  
**🍴** Fork 2.6k | **🔄** Updated 2026-08-27  
**👤** Pydantic — Pydantic AI 与 Pydantic AI Harness 团队

> Pydantic 的类型化 Python AI SDK 与 Agent Harness，统一支持多模型 Agent、实时语音、图像、embedding 和结构化输出，并以可组合能力提供记忆、子 Agent、上下文管理与长周期 coding workflow。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 15. PrimeIntellect-ai/prime-agent (19k ⭐)

**🔗** https://github.com/PrimeIntellect-ai/prime-agent  
**🍴** Fork 2k | **🔄** Updated 2026-08-27  
**👤** Prime Intellect — Prime Agent 与开放 Agent 研究基础设施团队

> Prime Intellect 开源的自改进 coding 与 research Agent，以 Recursive Language Model 和 Continual Harness 组织持久 REPL、子 Agent、记忆、Skills 与可回滚的经验精炼，面向长周期自主任务。

**匹配能力**：🧪 TDD 工作流 · 🤖 Agent 集成

---

## 16. hardikpandya/stop-slop (16k ⭐)

**🔗** https://github.com/hardikpandya/stop-slop  
**🍴** Fork 1.2k | **🔄** Updated 2026-03-17  
**👤** Hardik Pandya — 独立开发者，专注去除 AI 写作痕迹

> 去除 AI 写作痕迹的技能文件，识别并移除 LLM 可预测的写作模式。

**匹配能力**：🤖 Agent 集成

---

## 17. mindfold-ai/Trellis (14k ⭐)

**🔗** https://github.com/mindfold-ai/Trellis  
**🍴** Fork 798 | **🔄** Updated 2026-08-27  
**👤** mindfold-ai — Trellis agent harness 团队

> 高性能 Agent Harness 运行时，提供完整 agentic 工作流与多工具集成，文档见 docs.trytrellis.app。

**匹配能力**：🤖 Agent 集成

---

## 18. cobusgreyling/loop-engineering (11k ⭐)

**🔗** https://github.com/cobusgreyling/loop-engineering  
**🍴** Fork 1.5k | **🔄** Updated 2026-08-27  
**👤** Cobus Greyling — AI 与语言技术博主，Loop Engineering 方法论作者

> Loop Engineering 实践模式与 CLI 工具集，帮你设计「代替你反复 prompt agent」的自主循环系统（受 Addy Osmani、Boris Cherny 启发），含 loop-init / loop-audit / loop-cost 等脚手架与 Loop Ready 评分。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 19. frankbria/ralph-claude-code (9.6k ⭐)

**🔗** https://github.com/frankbria/ralph-claude-code  
**🍴** Fork 721 | **🔄** Updated 2026-07-18  
**👤** Frank Bria — ralph-claude-code 自主开发循环作者

> 面向 Claude Code 的自主 AI 开发循环，含智能退出检测，可持续运行直至任务完成。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 20. open-gsd/gsd-core (8.8k ⭐)

**🔗** https://github.com/open-gsd/gsd-core  
**🍴** Fork 624 | **🔄** Updated 2026-08-27  
**👤** open-gsd — GSD 工作流开源团队

> Git. Ship. Done 工作流 Harness 核心，Spec 驱动的轻量 agentic 交付闭环。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 21. gsd-build/gsd-2 (7.8k ⭐)

**🔗** https://github.com/gsd-build/gsd-2  
**🍴** Fork 765 | **🔄** Updated 2026-05-22  
**👤** TÂCHES / gsd-build — GSD 方法论作者

> 轻量 meta-prompting 与 Spec 驱动开发系统，支持 agent 长时间自主工作而不丢失全局视角。

**匹配能力**：🤖 Agent 集成

---

## 22. huangruiteng/loopx (5.2k ⭐)

**🔗** https://github.com/huangruiteng/loopx  
**🍴** Fork 465 | **🔄** Updated 2026-08-27  
**👤** 黄睿腾（huangruiteng）— 字节跳动 AML 工程师，loopx 作者

> 面向长周期 AI Agent 团队的轻量状态内核，提供持久目标、配额感知自动唤醒、可执行待办、证据日志与可验证交接，兼容 Codex、Claude Code 等多种 Agent Loop。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 23. gotalab/cc-sdd (3.6k ⭐)

**🔗** https://github.com/gotalab/cc-sdd  
**🍴** Fork 279 | **🔄** Updated 2026-05-20  
**👤** gotalab — cc-sdd Spec 驱动开发 Harness 团队

> 将已批准 spec 转化为长周期自主实现的最小 SDD Harness，含 Agent Skills，兼容 Claude Code、Codex、Cursor 等。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 24. mikeyobrien/ralph-orchestrator (3.1k ⭐)

**🔗** https://github.com/mikeyobrien/ralph-orchestrator  
**🍴** Fork 291 | **🔄** Updated 2026-08-25  
**👤** Mike O'Brien — ralph-orchestrator 自主 agent 编排作者

> Ralph Wiggum 自主 AI agent 编排的改进实现，支持多后端与长周期任务循环直至 spec 完成。

**匹配能力**：📋 Spec 驱动 · 🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 25. Forward-Future/loopy (3.1k ⭐)

**🔗** https://github.com/Forward-Future/loopy  
**🍴** Fork 276 | **🔄** Updated 2026-07-26  
**👤** Forward Future — Loop Library 与 Loopy Skill 维护团队

> 可复用 Agent Loop 目录与配套 Skill，帮助 agent 发现、审计、设计、执行和复盘带反馈的工作流。每个 loop 明确定义验收检查、下一步动作与停止条件。

**匹配能力**：🔀 多 Agent 编排 · 🧪 TDD 工作流 · 🤖 Agent 集成

---

## 26. michaelshimeles/ralphy (3k ⭐)

**🔗** https://github.com/michaelshimeles/ralphy  
**🍴** Fork 367 | **🔄** Updated 2026-02-05  
**👤** Michael Shimeles — ralphy 多 agent 自主循环脚本作者

> Ralph Wiggum 式自主 bash 循环，可驱动 Claude Code、Codex、Cursor 等 agent 直至 PRD 完成。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 27. rpamis/comet (2.9k ⭐)

**🔗** https://github.com/rpamis/comet  
**🍴** Fork 279 | **🔄** Updated 2026-08-27  
**👤** rpamis — Comet 长周期 Agent 工作流平台维护团队

> 可恢复的长周期 coding workflow 与 Skill 平台，把 OpenSpec、Superpowers、执行、验证、评估和发布连接成闭环。以状态机和 phase guard 保存进度，并用证据约束阶段退出。

**匹配能力**：📋 Spec 驱动 · 🔀 多 Agent 编排 · 🧪 TDD 工作流 · 🤖 Agent 集成

---

## 28. humanlayer/advanced-context-engineering-for-coding-agents (2.6k ⭐)

**🔗** https://github.com/humanlayer/advanced-context-engineering-for-coding-agents  
**🍴** Fork 194 | **🔄** Updated 2026-08-04  
**👤** HumanLayer — ACE for coding agents 维护团队

> Advanced Context Engineering（ACE）for coding agents：长上下文窗口下的检索、压缩与编排策略，提升自主开发可靠性。

**匹配能力**：🤖 Agent 集成

---

## 29. subsy/ralph-tui (2.4k ⭐)

**🔗** https://github.com/subsy/ralph-tui  
**🍴** Fork 235 | **🔄** Updated 2026-08-27  
**👤** subsy — Ralph TUI agent 循环编排工具作者

> AI Agent 循环编排 TUI：连接 Claude Code、OpenCode、Cursor CLI 等 agent 与任务列表，逐条自主执行并可视化进度。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 30. maxritter/pilot-shell (2.1k ⭐)

**🔗** https://github.com/maxritter/pilot-shell  
**🍴** Fork 176 | **🔄** Updated 2026-08-26  
**👤** Max Ritter — pilot-shell 生产级 agent 工作流作者

> 工程师向 Claude Code/Codex 工作流 Shell：Spec 规划、强制 TDD、持久记忆与多层质量门禁，让 agent 达到生产就绪。

**匹配能力**：🤖 Agent 集成

---

## 31. 777genius/agent-teams-ai (2k ⭐)

**🔗** https://github.com/777genius/agent-teams-ai  
**🍴** Fork 336 | **🔄** Updated 2026-08-26  
**👤** Ilya（777genius）— Agent Teams AI 作者，专注多 Agent 协作工具

> 多 Agent 团队桌面应用，通过任务看板、Agent 间通信和交叉审查组织自主协作。支持 Codex、Claude Code、OpenCode、Cursor 等多种 coding agent 与模型提供方。

**匹配能力**：🤖 Agent 集成

---

## 32. Th0rgal/open-ralph-wiggum (1.9k ⭐)

**🔗** https://github.com/Th0rgal/open-ralph-wiggum  
**🍴** Fork 142 | **🔄** Updated 2026-06-02  
**👤** Th0rgal — open-ralph-wiggum Ralph 循环 CLI 作者

> OpenCode / Claude Code / Codex 上的 Ralph 循环 CLI：`ralph "prompt"` 启动迭代，支持 prompt 文件与状态检查。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 33. the-open-engine/zeroshot (1.8k ⭐)

**🔗** https://github.com/the-open-engine/zeroshot  
**🍴** Fork 160 | **🔄** Updated 2026-08-26  
**👤** The Open Engine Company — Zeroshot 自主工程编排团队

> 自主软件工程多 Agent 编排 CLI，将实现者与独立 verifier 分离。验证失败会返回可复现问题并继续迭代，运行状态写入持久化 ledger 以支持恢复。

**匹配能力**：🤖 Agent 集成

---

## 34. ghuntley/how-to-ralph-wiggum (1.7k ⭐)

**🔗** https://github.com/ghuntley/how-to-ralph-wiggum  
**🍴** Fork 147 | **🔄** Updated 2026-01-11  
**👤** Geoffrey Huntley — Ralph Wiggum 方法论提出者

> Ralph Wiggum 方法论主文档：从 idea → spec → IMPLEMENTATION_PLAN → 自主循环，强调磁盘状态与 backpressure 验证。

**匹配能力**：📋 Spec 驱动 · 🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 35. Priivacy-ai/spec-kitty (1.6k ⭐)

**🔗** https://github.com/Priivacy-ai/spec-kitty  
**🍴** Fork 156 | **🔄** Updated 2026-08-27  
**👤** Priivacy-ai — spec-kitty SDD 工具团队

> 面向严肃开发者的 Spec 驱动开发工具：Kanban 看板、git worktree、自动合并，支持 Claude、Cursor、Gemini、Codex。

**匹配能力**：📋 Spec 驱动 · 🤖 Agent 集成

---

## 36. uber/ADR (1.5k ⭐)

**🔗** https://github.com/uber/ADR  
**🍴** Fork 136 | **🔄** Updated 2026-08-26  
**👤** Uber Open Source — ADR 企业 Agent 安全系统团队

> Uber 开源的企业 Agent 安全系统，通过遥测观测、安全基准和双层威胁检测分析 Cursor、Claude Code、Codex 等 Agent 的活动；仓库包含 ADR Sensor、ADR-Bench 与 ADR Detector。

**匹配能力**：🤖 Agent 集成

---

## 37. umputun/ralphex (1.5k ⭐)

**🔗** https://github.com/umputun/ralphex  
**🍴** Fork 119 | **🔄** Updated 2026-08-26  
**👤** Umputun — ralphex 扩展 Ralph 循环作者

> 扩展版 Ralph 循环 CLI，面向自主 AI 驱动的 plan 执行，支持多 agent 后端与完成信号检测。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 38. AnandChowdhary/continuous-claude (1.4k ⭐)

**🔗** https://github.com/AnandChowdhary/continuous-claude  
**🍴** Fork 92 | **🔄** Updated 2026-08-24  
**👤** Anand Chowdhary — Pabio 联合创始人，continuous-claude 作者

> 带 PR 的 Ralph 循环：Claude Code 持续自主运行、创建 PR、等待 CI 检查并合并。

**匹配能力**：🤖 Agent 集成

---

## 39. modu-ai/moai-adk (1.2k ⭐)

**🔗** https://github.com/modu-ai/moai-adk  
**🍴** Fork 219 | **🔄** Updated 2026-08-27  
**👤** modu-ai — moai-adk Agentic 开发套件团队

> SPEC 优先 Agentic 开发套件：24 个 AI agent + 52 个 skills，含 TDD/DDD 质量门禁，Go CLI、零依赖。

**匹配能力**：🤖 Agent 集成

---

## 40. open-gsd/gsd-pi (1.2k ⭐)

**🔗** https://github.com/open-gsd/gsd-pi  
**🍴** Fork 100 | **🔄** Updated 2026-08-27  
**👤** open-gsd — GSD-Pi 维护团队

> GSD meta-prompting 与 Spec 驱动开发系统（Pi 版），支持 agent 长时间自主运行并保持全局上下文。

**匹配能力**：🤖 Agent 集成

---

## 41. alchaincyf/loop-engineering-orange-book (1.1k ⭐)

**🔗** https://github.com/alchaincyf/loop-engineering-orange-book  
**🍴** Fork 108 | **🔄** Updated 2026-08-23  
**👤** 花叔（alchaincyf）— AI Native Coder，Loop Engineering 橙皮书作者

> 中英双语 Loop Engineering 通俗指南，解释 prompt、context、harness 与外层 loop 的关系，以及循环设计、验证、成本和落地方法。

**匹配能力**：🔀 多 Agent 编排 · 🤖 Agent 集成

---

## 42. ClaytonFarr/ralph-playbook (1k ⭐)

**🔗** https://github.com/ClaytonFarr/ralph-playbook  
**🍴** Fork 266 | **🔄** Updated 2026-03-06  
**👤** Clayton Farr — Ralph playbook 作者

> 基于 Geoff Huntley Ralph 方法论的自主 AI coding 循环实操 playbook，覆盖 spec、plan 与 loop 运行要点。

**匹配能力**：📋 Spec 驱动 · 🔀 多 Agent 编排 · 🤖 Agent 集成

---
## 分类索引

### 📋 Spec 驱动开发

- [spec-kit](#5-githubspec-kit-132k-) — Spec-Driven Development 入门工具包，帮助开发者快速上手 spec 驱动开发方法论
- [OpenSpec](#9-fission-aiopenspec-66k-) — 面向 AI coding assistant 的 Spec-Driven Development（SDD）框架
- [BMAD-METHOD](#10-bmad-code-orgbmad-method-52k-) — Breakthrough Method for Agile AI Driven Development，AI 驱动敏捷开…
- [gsd-core](#20-open-gsdgsd-core-88k-) — Git. Ship. Done 工作流 Harness 核心，Spec 驱动的轻量 agentic 交付闭环
- [gsd-2](#21-gsd-buildgsd-2-78k-) — 轻量 meta-prompting 与 Spec 驱动开发系统，支持 agent 长时间自主工作而不丢失全局视角
- [cc-sdd](#23-gotalabcc-sdd-36k-) — 将已批准 spec 转化为长周期自主实现的最小 SDD Harness，含 Agent Skills，兼容 Claude…
- [comet](#27-rpamiscomet-29k-) — 可恢复的长周期 coding workflow 与 Skill 平台，把 OpenSpec、Superpowers、执行…
- [spec-kitty](#35-priivacy-aispec-kitty-16k-) — 面向严肃开发者的 Spec 驱动开发工具：Kanban 看板、git worktree、自动合并，支持 Claude、C…
- [gsd-pi](#40-open-gsdgsd-pi-12k-) — GSD meta-prompting 与 Spec 驱动开发系统（Pi 版），支持 agent 长时间自主运行并保持全局…

### 🔁 Agent 迭代循环

- [superpowers](#1-obrasuperpowers-278k-) — 完整的 coding agent 软件开发方法论与技能框架
- [ralph](#13-snarktankralph-22k-) — 自主 Agent 循环框架：按 PRD 条目反复执行直至全部完成，适合长周期无人值守 feature 交付
- [loop-engineering](#18-cobusgreylingloop-engineering-11k-) — Loop Engineering 实践模式与 CLI 工具集，帮你设计「代替你反复 prompt agent」的自主循环…
- [ralph-claude-code](#19-frankbriaralph-claude-code-96k-) — 面向 Claude Code 的自主 AI 开发循环，含智能退出检测，可持续运行直至任务完成
- [loopx](#22-huangruitengloopx-52k-) — 面向长周期 AI Agent 团队的轻量状态内核，提供持久目标、配额感知自动唤醒、可执行待办、证据日志与可验证交接，兼容…
- [ralph-orchestrator](#24-mikeyobrienralph-orchestrator-31k-) — Ralph Wiggum 自主 AI agent 编排的改进实现，支持多后端与长周期任务循环直至 spec 完成
- [loopy](#25-forward-futureloopy-31k-) — 可复用 Agent Loop 目录与配套 Skill，帮助 agent 发现、审计、设计、执行和复盘带反馈的工作流
- [ralphy](#26-michaelshimelesralphy-3k-) — Ralph Wiggum 式自主 bash 循环，可驱动 Claude Code、Codex、Cursor 等 agen…
- [comet](#27-rpamiscomet-29k-) — 可恢复的长周期 coding workflow 与 Skill 平台，把 OpenSpec、Superpowers、执行…
- [ralph-tui](#29-subsyralph-tui-24k-) — AI Agent 循环编排 TUI：连接 Claude Code、OpenCode、Cursor CLI 等 agent…
- [open-ralph-wiggum](#32-th0rgalopen-ralph-wiggum-19k-) — OpenCode / Claude Code / Codex 上的 Ralph 循环 CLI：`ralph "promp…
- [how-to-ralph-wiggum](#34-ghuntleyhow-to-ralph-wiggum-17k-) — Ralph Wiggum 方法论主文档：从 idea → spec → IMPLEMENTATION_PLAN → 自主…
- [ralphex](#37-umputunralphex-15k-) — 扩展版 Ralph 循环 CLI，面向自主 AI 驱动的 plan 执行，支持多 agent 后端与完成信号检测
- [continuous-claude](#38-anandchowdharycontinuous-claude-14k-) — 带 PR 的 Ralph 循环：Claude Code 持续自主运行、创建 PR、等待 CI 检查并合并
- [loop-engineering-orange-book](#41-alchaincyfloop-engineering-orange-book-11k-) — 中英双语 Loop Engineering 通俗指南，解释 prompt、context、harness 与外层 loo…
- [ralph-playbook](#42-claytonfarrralph-playbook-1k-) — 基于 Geoff Huntley Ralph 方法论的自主 AI coding 循环实操 playbook，覆盖 spe…

### 🤖 Skills 与工作流

- [ECC](#2-affaan-mecc-244k-) — Agent Harness 性能优化全栈系统，涵盖 Skills、本能、记忆、安全与研究优先开发流程，面向 Claude…
- [skills](#3-mattpocockskills-238k-) — Matt Pocock 的个人 skills 目录，直接从 .claude 目录整理，面向真正的工程师
- [andrej-karpathy-skills](#4-multica-aiandrej-karpathy-skills-208k-) — 基于 Andrej Karpathy 对 LLM 编程陷阱的观察整理的单文件 CLAUDE.md 指南，以「先思考再编码…
- [agent-skills](#7-addyosmaniagent-skills-90k-) — 生产级 AI coding agent 工程技能集合，覆盖现代工程实践关键场景
- [taste-skill](#8-leonxlnxtaste-skill-81k-) — 开源设计品味 Skill，为 AI coding agent 提供更好的布局、排版、间距与动效审美
- [compound-engineering-plugin](#12-everyinccompound-engineering-plugin-25k-) — Every 的 Compound Engineering 官方插件，以可复用 Skills 将研究、规划、实现、审查和经…
- [prime-agent](#15-primeintellect-aiprime-agent-19k-) — Prime Intellect 开源的自改进 coding 与 research Agent，以 Recursive L…
- [stop-slop](#16-hardikpandyastop-slop-16k-) — 去除 AI 写作痕迹的技能文件，识别并移除 LLM 可预测的写作模式
- [cc-sdd](#23-gotalabcc-sdd-36k-) — 将已批准 spec 转化为长周期自主实现的最小 SDD Harness，含 Agent Skills，兼容 Claude…
- [moai-adk](#39-modu-aimoai-adk-12k-) — SPEC 优先 Agentic 开发套件：24 个 AI agent + 52 个 skills，含 TDD/DDD 质…

### ⚡ 优化与方法论

- [ECC](#2-affaan-mecc-244k-) — Agent Harness 性能优化全栈系统，涵盖 Skills、本能、记忆、安全与研究优先开发流程，面向 Claude…
- [autoresearch](#6-karpathyautoresearch-95k-) — 单 GPU nanochat 训练的自主 AI 研究循环：agent 修改 train.py、跑 5 分钟实验、用 va…
- [planning-with-files](#11-othmanadiplanning-with-files-26k-) — 面向 AI coding agent 的持久化文件规划 Skill
- [pydantic-ai](#14-pydanticpydantic-ai-20k-) — Pydantic 的类型化 Python AI SDK 与 Agent Harness，统一支持多模型 Agent、实时…
- [prime-agent](#15-primeintellect-aiprime-agent-19k-) — Prime Intellect 开源的自改进 coding 与 research Agent，以 Recursive L…
- [Trellis](#17-mindfold-aitrellis-14k-) — 高性能 Agent Harness 运行时，提供完整 agentic 工作流与多工具集成，文档见 docs.trytre…
- [gsd-core](#20-open-gsdgsd-core-88k-) — Git. Ship. Done 工作流 Harness 核心，Spec 驱动的轻量 agentic 交付闭环
- [cc-sdd](#23-gotalabcc-sdd-36k-) — 将已批准 spec 转化为长周期自主实现的最小 SDD Harness，含 Agent Skills，兼容 Claude…
- [advanced-context-engineering-for-coding-agents](#28-humanlayeradvanced-context-engineering-for-coding-agents-26k-) — Advanced Context Engineering（ACE）for coding agents：长上下文窗口下的检…
- [pilot-shell](#30-maxritterpilot-shell-21k-) — 工程师向 Claude Code/Codex 工作流 Shell：Spec 规划、强制 TDD、持久记忆与多层质量门禁，…
- [agent-teams-ai](#31-777geniusagent-teams-ai-2k-) — 多 Agent 团队桌面应用，通过任务看板、Agent 间通信和交叉审查组织自主协作
- [zeroshot](#33-the-open-enginezeroshot-18k-) — 自主软件工程多 Agent 编排 CLI，将实现者与独立 verifier 分离
- [ADR](#36-uberadr-15k-) — Uber 开源的企业 Agent 安全系统，通过遥测观测、安全基准和双层威胁检测分析 Cursor、Claude Cod…
- [loop-engineering-orange-book](#41-alchaincyfloop-engineering-orange-book-11k-) — 中英双语 Loop Engineering 通俗指南，解释 prompt、context、harness 与外层 loo…


---

## 收录规则

- 收录标准：wind8ai 已 star 且在该类目 Star List 中即收录，按 GitHub Star 总数降序
- 正式榜上限 **42** 个；超出上限的 eligible 条目见 [备选榜](./README.overflow.md)
- 数据来源：wind8ai [Star List `loop-engineering`](https://github.com/stars/wind8ai/lists/loop-engineering)

---

## 许可

本榜策展正文（排行、描述、分类索引）采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) © wind8ai。  
各条目所链接的开源项目受其各自仓库许可证约束。

---

*由 wind8 整理 | 2026-08-27*
